...
def error_analysis(true_labels, preds, output_file_path, colors=['red', 'green', 'blue', 'orange', 'lime', 'cyan']):
    error_dict = dict()
    ...
    for true_label in config.LABELS:
        ...
        for pred in config.LABELS:
            ...
            plt.bar(x + unit_length * sub_index, freq, width=0.1, color=colors[sub_index], label='Predicted as ' + pred)
            sub_index += 1
        index += 1
