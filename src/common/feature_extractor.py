...
def extract_features(input_file_path_list, transformed_vectorizer):
    ...
    if transformed_vectorizer is None:
        vectorizer = CountVectorizer(min_df=50, stop_words='english', binary=True)
...
def extract(base_input_dir_path, data_type, vectorizer, base_output_dir_path):
    ...
    transformed_vectorizer, feature_mat, header_feature_mat = extract_features(input_file_path_list, vectorizer)
...
def main(args):
    transformed_vectorizer = extract(args.input, config.TRAINING, None, args.output)
    extract(args.input, config.VALIDATION, transformed_vectorizer, args.output)
    extract(args.input, config.TEST, transformed_vectorizer, args.output)
